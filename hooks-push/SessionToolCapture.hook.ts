...
try {
  ...
  // 处理空输入
  if (!input || input.trim() === '') {
    console.error('[SessionToolCapture] Warning: Empty stdin received, skipping');
  ...
  // 验证必需字段
  if (!event.session_id) {
    console.error('[SessionToolCapture] Warning: Missing session_id in input');
    console.log(JSON.stringify({ continue: true }));
    process.exit(0);
  }
} catch (error) {
  console.error('[SessionToolCapture] Error parsing input:', error instanceof Error ? error.message : String(error));
  console.log(JSON.stringify({ continue: true }));
  process.exit(0);
}
